...
class RendererPytorch3D:
    def __init__(self, lights=None, camera=None, dim=(224, 224)):

        self.raster_settings = RasterizationSettings(
            image_size=dim, blur_radius=0.0, faces_per_pixel=1
        )
    ...
    def render_front_views(
        ...
        pytorch3d_mesh = Meshes(
            [verts for i in range(num_views)],
            [faces for i in range(num_views)],
            textures,
        )
