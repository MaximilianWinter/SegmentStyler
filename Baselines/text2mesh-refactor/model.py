...
from helpers import get_resolution, get_neural_style_network
...
class Text2Mesh(pl.LightningModule):
    def __init__(self, args):
        ...
        self.clip_model, self.preprocess, self.clip_transform = get_clip(self.args, self.res)
        self.augment_transform, self.normaugment_transform, \
        self.displaugment_transform = get_augumentations(self.args, self.res, self.clip_normalizer)
    ...
    def configure_optimizers(self):
        optim = torch.optim.Adam(self.mlp.parameters(), self.args.learning_rate, weight_decay=self.args.decay)
        activate_scheduler = self.args.lr_decay < 1 and self.args.decay_step > 0 and not self.args.lr_plateau
        
        lr_scheduler = None
        if activate_scheduler:
            lr_scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=self.args.decay_step, gamma=self.args.lr_decay)
